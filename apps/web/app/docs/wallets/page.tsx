...
message      { hash: <the permission's hash, from SpendPermissionManager.getHash> }`}</Pre>
      ...
      <P>
        ...
        signature first and skips the deployment. One path, new or returning. <Cite file="apps/web/app/api/permissions/route.js" line={142} />
      </P>
      ...
      <P>
        ...
        verified delegator below is told plainly instead of being asked to sign into a failure. <Cite file="apps/web/components/sign/sign-flow.tsx" line={183} /> The server checks again before
      ...
      <P>
        ...
        whatever their account has become. <Cite file="apps/web/components/sign/sign-flow.tsx" line={184} /> A new registration from an upgraded account was
      ...
      <P>
        ...
        <Cite file="apps/web/components/sign/sign-flow.tsx" line={429} />; re-runs every check when the wallet reports a different account, so a refusal for
        one account never lingers for another <Cite file="apps/web/components/sign/sign-flow.tsx" line={235} />; and offers MetaMask&apos;s own account picker
        from the refusal <Cite file="apps/web/components/sign/sign-flow.tsx" line={248} />. That behaviour is checked in a browser, with a negative control
      ...
      <Callout kind="note" title="A deliberate, scoped exception: MetaMask's delegator">
        ...
        <Cite file="apps/web/app/api/permissions/route.js" line={109} />
        <br /><br />
      ...
      <UL>
        ...
        <li>The sign button stays disabled until the hash computed on the page equals the manager&apos;s own <C>getHash</C>, read from the chain. <Cite file="apps/web/components/sign/sign-flow.tsx" line={369} /> <Cite file="apps/web/components/sign/sign-flow.tsx" line={371} /></li>
        <li>That same hash is shown as the fingerprint the wallet will display, with an instruction not to sign if it differs — which turns a blind signature into one the customer can check. <Cite file="apps/web/components/sign/sign-flow.tsx" line={523} /></li>
      ...
      <P>
        ...
        from the same step. <Cite file="apps/web/components/sign/sign-flow.tsx" line={276} />
        <Cite file="apps/web/components/sign/sign-flow.tsx" line={478} />
      </P>
      ...
      <Table head={["Refusal", "What it catches"]} rows={[
        [<C key="a">wrong_domain</C>, <>Typed data signed under the wrong domain version. <Cite key="a1" file="apps/web/app/api/permissions/route.js" line={135} /></>],
        [<C key="b">personal_sign</C>, <>The hash signed as a plain message rather than typed data. <Cite key="b1" file="apps/web/app/api/permissions/route.js" line={131} /></>],
        [<C key="c">unwrapped_hash</C>, <>The bare permission hash, without the account&apos;s replay-safe wrapper — which would be valid for any account. <Cite key="c1" file="apps/web/app/api/permissions/route.js" line={133} /></>],
        [<C key="d">not_owner</C>, <>Correctly formed typed data from a key that does not own the account. <Cite key="d1" file="apps/web/app/api/permissions/route.js" line={126} /></>],
        ...
        [<C key="i">rate_limited</C>, <>A second registration from one signer inside the cooldown, or too many from one client or overall. <Cite key="i1" file="apps/web/app/api/permissions/route.js" line={160} /></>],
      ]} />
      <P>
        ...
        <Cite file="apps/web/app/api/permissions/policy.js" line={26} /> <Cite file="apps/web/app/api/permissions/route.js" line={177} />
      </P>
      ...
      <P>
        ...
        <C>account.execute(manager, revoke(permission))</C>. <Cite file="apps/web/components/sign/sign-flow.tsx" line={325} /> The drill sends
      ...
      <UL>
        ...
        <li><b>A stranger, on the public site.</b> Registration needs the executor key, which is deliberately not deployed, and there is no hosted worker to run charges. Both are one pending, deliberate step. Until then the page says so before anyone signs. <Cite file="apps/web/components/sign/sign-flow.tsx" line={389} /></li>
